...
- Edit [apps/server/src/auth/auth.ts](apps/server/src/auth/auth.ts) to add the `jwt()` plugin and `oauthProvider({ issuer, scopes, validAudiences, loginPage, consentPage })`. Use `env.APP_BASE_URL + "/api/auth"` as the issuer. Do not set `mal-cli` as a cached trusted client; it is a public OAuth client and must require user consent.
