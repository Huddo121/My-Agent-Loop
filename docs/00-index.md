# My Agent Loop

## About this project

My Agent Loop is a project that aims to make it easier to allow agents to more autonomously iterate on software projects.

- [Index](./00-index.md): This file, useful to determine which other files are worth reading
- [Introduction](./01-introduction.md): Contains some high level information about what the My Agent Loop project is doing, and how it does it
- [Coding Practices](./02-coding-practices.md): Specific advice on how to write, structure, and verify code in this codebase
- [Test fakes](./04-test-fakes.md): Using in-memory fakes vs mocks for `@mono/server` tests (test doubles, when to use which)
- [Concepts](./03-concepts.md): More detailed information about the high level concepts in use in this project
- [Decisions](./decisions): Architecture and design decisions
  - [Codex OAuth via mal](./decisions/codex-oauth-via-mal-cli.md): Local CLI OAuth flow for Codex subscription auth and server-side user OAuth credential storage
- [Ideas](./ideas): This is a folder of notes for things that I'm thinking about but not yet working on, or may never work on
